...
新建 scratch session 会写入：

```text
.aiw-session.json
```

字段包括创建时间、agent、session id、第一条消息和路径。`aiw scratch resume` 会扫描 `paths.sessions` 下的日期/会话目录，用 fzf 展示：

```text
时间    session id    第一条消息    路径
```

因此可以直接输入日期、session id、第一条消息片段或路径片段进行模糊搜索。自动化场景可用：

```bash
aiw scratch list --json
aiw scratch resume --id <session-id> --dry-run
```
...
这些 action 进入 plus-button context menu，不改变默认 plus-button 的 `aiw-new-worktree` 主路径。
