...
- 新增 `.github/workflows/ci.yml`：PR/master 执行测试、vet 和四平台构建；`v*` tag 在验证成功后创建同名 GitHub Release，上传版本化归档及 SHA-256 清单。tag 会通过 `AIW_VERSION` 将版本注入二进制。内部制品库镜像尚未配置，且本地验证不等于远端 Release 已发布。设计记录见 `docs/2026-09-20-release-pipeline.md`。
